 ...
 *
 * the addition here is the option to use environment variables,
 * if not using change "backdrop" to the respective variable value for your db
 *
 * these datbase settings have the 4-byte UTF-8 encoding enabled
 * make sure your mysql custom.cnf is set
 * https://api.backdropcms.org/database-configuration
 */
 
$dbuser = getenv('BACKDROP_DB_USER') ? getenv('BACKDROP_DB_USER') : "backdrop";
$dbuserpass = getenv('BACKDROP_DB_PASSWORD') ? getenv('BACKDROP_DB_PASSWORD') : "backdrop";
$dbserver = getenv('BACKDROP_DB_HOST') ? getenv('BACKDROP_DB_HOST') : "backdrop";
$dbname = getenv('BACKDROP_DB_NAME') ? getenv('BACKDROP_DB_NAME') : "backdrop";

$database = 'mysql://'. $dbuser .':'. $dbuserpass .'@'. $dbserver .'/'. $dbname;
$database_prefix = '';
